"""SH Billboard pipeline — GPU-accelerated SH glyph rendering for DIPY."""
...
class BillboardSphGlyphPrecomputeShader(BaseShader):
    ...
    def get_code(self):
        return load_dipy_wgsl("sh_precompute.wgsl")
...
class BillboardSphGlyphShader(MeshShader):
    ...
    def get_render_info(self, wobject, shared):
        try:
            ...
            if isinstance(mat, _SphGlyphLutMaterial) and auto_detach and lut_ready:
...
def _populate_radius_lut_cpu(actor, phi_res, theta_res, glyph_count, n_coeffs):
    ...
    z = np.cos(tt)
    dirs = np.column_stack([x.ravel(), y.ravel(), z.ravel()]).astype(np.float32)
...
def _attach_precomputed_radius_tables(actor, precompute_data):
    ...
    success = False
    try:
        _populate_radius_lut_cpu(actor, phi_res, theta_res, glyph_count, n_coeffs)
        success = True
    except Exception:
        pass
...
def sph_glyph_billboard(
    ...
    *,
    ...
    if coeffs_arr.ndim == 4:
        ...
        if centers is None:
            grid = np.indices(coeffs_arr.shape[:3], dtype=np.float32).reshape(3, -1).T
            centers_arr = grid
        else:
            centers_arr = np.asarray(centers, dtype=np.float32)
    elif coeffs_arr.ndim == 2:
        if centers is None:
            raise ValueError("centers required when coeffs is two-dimensional.")
        centers_arr = np.asarray(centers, dtype=np.float32)
    ...
    if centers_arr.shape[0] != coeffs_flat.shape[0]:
        raise ValueError("centers and coeffs must have matching glyph count.")
...
def create_large_scale_sh_billboards(
    ...
    **kwargs,
    ...
    if coeffs.ndim == 4:
        ...
        grid = np.indices(coeffs.shape[:3], dtype=np.float32).reshape(3, -1).T
        centers = grid
